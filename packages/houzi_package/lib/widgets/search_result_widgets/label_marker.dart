...
Future<BitmapDescriptor> createCustomMarkerBitmap(String title,
  ...
  RRect org_rect = RRect.fromLTRBAndCorners(0, 0, textWidth + 40, textHeight + 20,
  ...
  var shadownPath = Path();
  shadownPath.moveTo(5, 5);
  shadownPath.lineTo(textWidth + 40, 5);
  shadownPath.lineTo(textWidth + 40, textHeight + 20);
  shadownPath.lineTo(5, textHeight + 20);
  shadownPath.lineTo(5, 5);
  shadownPath.moveTo((textWidth + 40) / 2 - 15, textHeight + 20);
  shadownPath.lineTo((textWidth + 40) / 2, textHeight + 40);
  shadownPath.lineTo((textWidth + 40) / 2 + 15, textHeight + 20);
  shadownPath.close();
  ...
  RRect rRect = RRect.fromLTRBAndCorners(5, 5, textWidth + 40, textHeight + 20,
      bottomLeft: const Radius.circular(10),
      bottomRight: const Radius.circular(10),
      topLeft: const Radius.circular(10),
      topRight: const Radius.circular(10));

  canvas.drawShadow(shadownPath, Colors.black.withAlpha(255), 4.0, false);
  ...
  painter.layout();
  painter.paint(canvas, const Offset(22.0, 12.0));
  ui.Picture p = pictureRecorder.endRecording();
  ByteData? pngBytes = await (await p.toImage(
          painter.width.toInt() + 55, painter.height.toInt() + 65))
